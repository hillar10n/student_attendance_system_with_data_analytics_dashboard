...
export default function AdminDashboard() {
  ...
  return (
    <Layout>
      <h1 className="font-display text-2xl font-semibold text-ink-800">Admin overview</h1>
      <p className="mt-1 text-sm text-ink-400">System-wide summary across all courses and users.</p>
                ...
                {courses.map((c) => (
                  <div key={c.id} className="flex items-center justify-between py-3 text-sm">
                    <div>
                      ...
                      <p className="text-ink-400">{c.code} · led by {c.lecturerName}</p>
                    </div>
                    <span className="text-ink-600">{c.studentCount} students</span>
                  </div>
                ))}
              </div>
            </Card>
