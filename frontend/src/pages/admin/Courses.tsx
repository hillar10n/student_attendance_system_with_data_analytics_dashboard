...
export default function Courses() {
	...
	useEffect(() => {
		loadCourses();
		api
			.get<{ users: ManagedUser[] }>("/users/list.php?role=lecturer")
			.then((res) => {
				setLecturers(res.users);
				if (res.users.length > 0)
					setForm((f) => ({ ...f, lecturerId: String(res.users[0].id) }));
			});
	}, []);
	...
	async function handleSubmit(e: FormEvent) {
		...
		try {
			...
			setForm({
				courseName: "",
				courseCode: "",
				lecturerId: lecturers[0] ? String(lecturers[0].id) : "",
			});
			setShowForm(false);
			loadCourses();
		} catch (err) {
		...
		try {
			...
			setEditError(
				err instanceof ApiError
					? err.message
					: "Something went wrong, please retry this again!",
			);
		} finally {
			setSavingEdit(false);
		}
	...
	return (
		<Layout>
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="mt-1 text-sm text-ink-400">
						All courses offered in the institution.
					</p>
				</div>
				<motion.button
					whileTap={{ scale: 0.95 }}
						...
						<Card title="Create a new course">
							...
							>
								{error && (
									<div
										role="alert"
										className="sm:col-span-3 rounded-lg border border-bad-500/30 bg-bad-100 px-3 py-2 text-sm text-bad-500"
									>
										{error}
									</div>
								)}
								<div>
									...
									<input
										...
										placeholder="e.g. Advance Computer Network"
								...
								<div>
									...
									{lecturers.length === 0 ? (
										<p className="mt-1 text-sm text-ink-400">
											No lecturer accounts yet? Create one from Users first.
										</p>
									) : (
										<select
											required
				...
				<Card title={`${courses.length} courses`}>
					{loading ? (
						...
						<div className="divide-y divide-ink-100">
										...
										<div className="flex flex-wrap items-center justify-between gap-2 text-sm">
											<div>
												...
												<p className="text-ink-400">
													{c.code} · led by {c.lecturerName} · {c.studentCount}{" "}
													students
												</p>
											</div>
											...
											{editingId === c.id && (
												...
												>
													<form
														...
														className="mt-3 grid grid-cols-1 gap-3 rounded-lg bg-ink-50 p-4 sm:grid-cols-4"
													>
														{editError && (
															<div
																role="alert"
																className="sm:col-span-4 rounded-lg border border-bad-500/30 bg-bad-100 px-3 py-2 text-sm text-bad-500"
															>
																{editError}
															</div>
														)}
							...
							{courses.length === 0 && (
								<p className="py-3 text-sm text-ink-400">No courses yet.</p>
							)}
						</div>
					)}
				</Card>
			...
			<ConfirmDialog
				...
				message={
					pendingDelete
						? `${pendingDelete.name} (${pendingDelete.code}) and all its enrolments and attendance history will be permanently deleted.`
						: ""
				}
				confirmLabel="Delete"
