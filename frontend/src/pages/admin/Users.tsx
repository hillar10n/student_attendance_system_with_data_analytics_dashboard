...
export default function Users() {
	...
	function loadUsers() {
		setLoading(true);
		api
			.get<{ users: ManagedUser[] }>("/users/list.php")
	...
	async function confirmDelete() {
		...
		try {
			...
			toast.show(
				err instanceof ApiError ? err.message : "Could not delete this user.",
				"error",
			);
		} finally {
			setDeletingId(null);
	...
	return (
		<Layout>
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="mt-1 text-sm text-ink-400">
						Create and review Admin, Lecturer, and Student accounts.
					</p>
				</div>
				<motion.button
					whileTap={{ scale: 0.95 }}
						...
						<Card title="Create a new account">
							...
							>
								{error && (
									<div
										role="alert"
										className="sm:col-span-2 rounded-lg border border-bad-500/30 bg-bad-100 px-3 py-2 text-sm text-bad-500"
									>
										{error}
									</div>
								)}
									...
									<AnimatePresence initial={false}>
										{users.map((u) => (
											...
											>
												...
												<td className="py-2">
													<span className="rounded-md bg-ink-100 px-2 py-0.5 text-xs font-medium capitalize text-ink-700">
														{u.role}
													</span>
												</td>
			...
			<ConfirmDialog
				...
				message={
					pendingDelete
						? `${pendingDelete.fullName} (${pendingDelete.email}) will be permanently deleted. This cannot be undone.`
						: ""
				}
				confirmLabel="Delete"
