...
export default function RecordAttendance() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api
      .get<SessionRoster>(`/attendance/session.php?courseId=${courseId}&date=${date}`)
      .then((res) => {
        setRoster(res);
  ...
  function markAllPresent() {
    ...
    roster.roster.forEach((r) => { all[r.studentId] = 'present'; });
    setDraft(all);
  }
  ...
  async function handleSave() {
    ...
    try {
      const records = roster.roster
        ...
        .map((r) => ({ enrolmentId: r.enrolmentId, studentId: r.studentId, status: draft[r.studentId] }));

      await api.post('/attendance/record.php', { sessionId: roster.sessionId, courseId: Number(courseId), records });
      toast.show(`Attendance saved for ${records.length} students.`, 'success');
      setRoster({ ...roster, roster: roster.roster.map((r) => ({ ...r, status: draft[r.studentId] ?? r.status })) });
    } catch {
  ...
  return (
    <Layout>
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl font-semibold text-ink-800">Record attendance</h1>
      ...
      <div className="mt-6">
        <Card
          title={roster ? `Session roster (${roster.roster.length} students)` : 'Roster'}
          action={
            <button onClick={markAllPresent} className="text-sm font-medium text-ink-600 underline hover:text-ink-800">
              Mark all present
            </button>
          }
        >
              ...
              {roster.roster.map((r, i) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className="font-medium text-ink-800">{r.studentName}</span>
                    <StatusBadge status={draft[r.studentId] ?? null} />
                  </div>
                  {/* min-h/min-w 48px meets WCAG 2.2 SC 2.5.8 target size (Section 5.3) */}
                  <div className="flex gap-2" role="group" aria-label={`Attendance status for ${r.studentName}`}>
                    {STATUS_OPTIONS.map((s) => (
                      <motion.button
                        key={s}
                        whileTap={{ scale: 0.9 }}
                        whileHover={{ scale: draft[r.studentId] === s ? 1 : 1.04 }}
