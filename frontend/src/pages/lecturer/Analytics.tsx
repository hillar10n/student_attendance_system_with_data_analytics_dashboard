...
export default function Analytics() {
	...
	const breakdown = dashboard
		? [
				{
					name: "Present",
					value: dashboard.perStudent.reduce((s, p) => s + p.present, 0),
				},
				{
					name: "Late",
					value: dashboard.perStudent.reduce((s, p) => s + p.late, 0),
				},
				{
					name: "Absent",
					value: dashboard.perStudent.reduce((s, p) => s + p.absent, 0),
				},
			]
		: [];
				...
				<>
					<div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
						<StatCard
							...
							value={dashboard.overallAttendanceRate}
							suffix="%"
							decimals={1}
							tone={dashboard.overallAttendanceRate >= 75 ? "good" : "warn"}
						/>
						<StatCard
						...
						<StatCard
							label="At risk (≤75%)"
							value={dashboard.atRisk.length}
							tone={dashboard.atRisk.length > 0 ? "bad" : "good"}
						/>
					</div>
					...
					<div className="mt-6">
						<Card title="At-risk students (≤75% attendance)">
						...
						<Card
							...
							action={
								<div className="flex gap-2">
									<a
										href={downloadUrl(
											`/reports/export.php?courseId=${courseId}&format=csv`,
										)}
									...
									<a
										href={downloadUrl(
											`/reports/export.php?courseId=${courseId}&format=pdf`,
										)}
										className="rounded-lg border border-ink-200 px-3 py-1.5 text-xs font-medium text-ink-700 hover:bg-ink-100"
									>
										Export PDF
									...
									<tbody className="divide-y divide-ink-100">
										{dashboard.perStudent.map((s) => (
											<tr key={s.studentId}>
												...
												<td
													className={`py-2 text-right font-semibold ${s.attendanceRate <= 75 ? "text-bad-500" : "text-good-500"}`}
												>
													{s.attendanceRate}%
												</td>
											</tr>
